...
import com.cuit.exam.entity.TRole;
 ...
 * 
 * @see com.cuit.exam.entity.TRole
...
public class TRoleDAO {
	...
	public TRole findById(java.lang.Integer id) {
		...
		try {
			TRole instance = (TRole) getCurrentSession().get(
					"com.cuit.exam.entity.TRole", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<TRole> findByExample(TRole instance) {
		...
		try {
			List<TRole> results = (List<TRole>) getCurrentSession()
					.createCriteria("com.cuit.exam.entity.TRole")
	...
	public TRole merge(TRole detachedInstance) {
		...
		try {
			TRole result = (TRole) getCurrentSession().merge(detachedInstance);
